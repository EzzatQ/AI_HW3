...
class KFoldCrossValidation:
    ...
    def __init__(self, k, classifier, data_set: np.ndarray, shuffle=False, filename='KFCV',
                 early_pruning=False, tune_pruning_param=False, early_pruning_param=12,
                 early_pruning_tuning_options=None, eval_type="loss", entropy_param=3.5,
                 pruning_weight_param=1, split_by="information_gain"):
        self.k = k
    ...
    def getError(self, pruning_param=None):
        ...
        for train_idx, test_idx in self.kf.split(self.data_set):
            clf = self.classifier(filename=f"{self.filename}_{i}th_fold_classificationTree",
                                  early_pruning=self.early_pruning, early_pruning_param=pruning_param,
                                  entropy_param=self.entropy_param, pruning_weight_param=self.pruning_weight_param,
